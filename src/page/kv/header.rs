...
use crate::page::common::{
    PAGE_MAGIC, PAGE_VERSION_V3, PAGE_TYPE_KV_RH3, TRAILER_LEN,
    OFF_MAGIC, OFF_VERSION, OFF_TYPE, OFF_PAGE_ID,
    KV_HDR_MIN,
    KV_OFF_DATA_START, KV_OFF_TABLE_SLOTS, KV_OFF_USED_SLOTS, KV_OFF_FLAGS,
    KV_OFF_NEXT_PID, KV_OFF_LSN, KV_OFF_CODEC_ID,
};
...
pub fn kv_init_v3(page: &mut [u8], page_id: u64, codec_id: u16) -> Result<()> {
    ...
    // KV поля
    LittleEndian::write_u32(&mut page[KV_OFF_DATA_START..KV_OFF_DATA_START + 4], KV_HDR_MIN as u32);
...
pub fn kv_header_write_v3(page: &mut [u8], h: &KvHeaderV3) -> Result<()> {
    ...
    LittleEndian::write_u32(&mut page[KV_OFF_DATA_START..KV_OFF_DATA_START + 4], h.data_start);
    LittleEndian::write_u32(&mut page[KV_OFF_TABLE_SLOTS..KV_OFF_TABLE_SLOTS + 4], h.table_slots);
    LittleEndian::write_u32(&mut page[KV_OFF_USED_SLOTS..KV_OFF_USED_SLOTS + 4], h.used_slots);
    LittleEndian::write_u32(&mut page[KV_OFF_FLAGS..KV_OFF_FLAGS + 4], h.flags);
    LittleEndian::write_u64(&mut page[KV_OFF_NEXT_PID..KV_OFF_NEXT_PID + 8], h.next_page_id);
