...
//!   * stream_id      — идентификатор источника WAL (anti‑mix), читается из заголовка WAL.
//!
//! Формат файлов (LE, 8 байт):
//! - <root>/.heads_lsn.bin : [u64 last_heads_lsn]
//! - <root>/.cdc_seq.bin   : [u64 last_seq]
//! - <root>/.stream_id.bin : [u64 stream_id]
//!
//! Поведение:
...
//!
//! Замечание:
//! - Хранилище намеренно простое (один u64). Этого достаточно для LSN/seq/stream‑id гейтинга,
...
pub const STREAM_ID_FILE: &str = ".stream_id.bin";
...
#[inline]
pub fn stream_id_path(root: &Path) -> PathBuf {
    root.join(STREAM_ID_FILE)
}
...
/// Загрузить stream_id (0, если файл отсутствует).
/// Используется CDC apply для проверки источника и CLI-инструментами.
pub fn load_stream_id(root: &Path) -> Result<u64> {
    let p = stream_id_path(root);
    if !p.exists() {
        return Ok(0);
    }
    let mut f = OpenOptions::new()
...
/// Сохранить stream_id в файл.
/// Может вызываться при первом запуске writer’а или в apply после валидации источника.
pub fn store_stream_id(root: &Path, v: u64) -> Result<()> {
    let p = stream_id_path(root);
    let mut f = OpenOptions::new()
        .create(true)
        .write(true)
